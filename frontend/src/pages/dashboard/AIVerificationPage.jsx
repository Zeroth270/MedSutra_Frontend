...
export default function AIVerificationPage() {
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader title="AI Verification" subtitle="Scan and verify your medications with AI Vision" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        >
          <div className={`w-20 h-20 rounded-3xl flex items-center justify-center text-4xl transition-all ${dragging ? 'bg-yellow-100 border-yellow-300' : 'bg-yellow-50 border border-yellow-100'}`}>🔬</div>
          ...
          <button className="btn-primary px-7 py-3 rounded-xl text-sm">Choose File</button>
          ...
          <div className="space-y-6">
            {STEPS.map((s) => (
              <div key={s.step} className="flex gap-4">
                <div className="w-10 h-10 bg-gradient-to-br from-yellow-400 to-amber-500 rounded-xl flex items-center justify-center text-sm font-black text-black flex-shrink-0 shadow-md shadow-yellow-100">{s.step}</div>
                <div className="pt-1">
          ...
          {HISTORY.map((h, i) => (
            <div key={i} className="flex items-center justify-between px-7 py-4 hover:bg-gray-50/60 transition-colors">
              <div className="flex items-center gap-4">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg flex-shrink-0 ${h.ok ? 'bg-green-50' : 'bg-red-50'}`}>{h.ok ? '✅' : '⚠️'}</div>
                <div>
                  <p className="text-sm font-semibold text-gray-900">{h.med}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{h.date}</p>
                </div>
              </div>
              <span className={`text-xs font-bold px-3 py-1.5 rounded-full ${h.ok ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>{h.result}</span>
            </div>
          ))}
        </div>
      </div>
