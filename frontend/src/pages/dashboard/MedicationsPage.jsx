...
export default function MedicationsPage() {
  return (
    <div>
      <PageHeader title="Medications" subtitle="Manage and track all your prescriptions." actionLabel="Add Medication" />
      ...
      <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
        ...
        >
          <span>Medication</span><span>Frequency</span><span>Schedule</span><span>Stock</span><span>Status</span><span></span>
        </div>
        {meds.map((med, i) => {
          const isLow = med.status === 'Low Stock';
          return (
            <div key={i} className="grid px-6 py-4 border-b border-gray-50 last:border-0 items-center hover:bg-gray-50 transition-colors" style={{ gridTemplateColumns: '2fr 1fr 1fr 120px 100px 80px' }}>
              <div>
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 bg-gray-100 rounded-full h-1.5 max-w-14">
                  <div className={`h-1.5 rounded-full ${isLow ? 'bg-red-400' : 'bg-green-400'}`} style={{ width: `${Math.min((med.stock / 30) * 100, 100)}%` }} />
                </div>
                <span className="text-xs text-gray-500">{med.stock}</span>
              </div>
              <span className={`text-xs font-semibold px-2.5 py-1 rounded-full w-fit ${isLow ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-700'}`}>{med.status}</span>
              <button className="text-xs text-gray-400 hover:text-gray-700 font-medium hover:bg-gray-100 px-3 py-1.5 rounded-lg transition-all">Edit</button>
            </div>
          );
        })}
      </div>
