...
export default function RiskReportPage() {
  return (
    <div>
      <PageHeader title="Risk Report" subtitle="AI-powered analysis of your adherence patterns." />
            ...
            {factors.map((f, i) => (
              <div key={i}>
                ...
                <div className="bg-gray-100 rounded-full h-2">
                  <div className={`h-2 rounded-full ${f.barColor} transition-all duration-700`} style={{ width: `${(f.value / f.max) * 100}%` }} />
                </div>
            ...
            <div className="space-y-3">
              {tips.map((tip, i) => (
                <div key={i} className="flex gap-3 p-3.5 bg-gray-50 rounded-lg">
                  <div className="w-5 h-5 rounded bg-gray-900 flex items-center justify-center text-white text-xs font-black flex-shrink-0 mt-0.5">{i + 1}</div>
