...
export default function CaregiverLinkPage() {
  ...
  return (
    <div>
      <PageHeader title="Caregiver Link" subtitle="Share your health data with trusted contacts." actionLabel="Invite Caregiver" />
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {caregivers.map((c, i) => (
            <div key={i} className="bg-white border border-gray-100 rounded-xl p-5 flex items-center gap-4 hover:shadow-sm transition-all">
              <div className="w-10 h-10 rounded-lg bg-gray-900 flex items-center justify-center font-bold text-white text-sm flex-shrink-0">{c.initial}</div>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-900 text-sm">{c.name}</p>
                <p className="text-xs text-gray-400 mt-0.5">{c.role}</p>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-xs font-semibold bg-green-50 text-green-700 px-2.5 py-1 rounded-full">{c.status}</span>
                <button className="text-xs border border-gray-200 hover:bg-red-50 hover:text-red-500 hover:border-red-100 px-3 py-1.5 rounded-lg text-gray-400 transition-all font-medium">Remove</button>
              </div>
            </div>
          ))}
          <div className="bg-white border-2 border-dashed border-gray-200 hover:border-gray-400 rounded-xl p-5 flex items-center gap-4 cursor-pointer transition-all group">
            <div className="w-10 h-10 rounded-lg bg-gray-100 group-hover:bg-gray-200 flex items-center justify-center text-gray-400 text-xl flex-shrink-0 transition-all">+</div>
            <div>
