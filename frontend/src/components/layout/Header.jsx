    ...
    <header className="fixed top-0 left-0 right-0 z-50 theme-surface shadow-sm h-20">
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-full">
        <Link to={ROUTES.HOME} className="flex items-center gap-2.5 no-underline group">
          ...
          <span className="theme-text font-black text-lg tracking-tight uppercase group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors duration-300">MedSutra AI</span>
        </Link>
        ...
        <div className="flex items-center gap-4">
          ...
          {isAuthenticated ? (
            <div className="flex items-center gap-3">
              ...
              <button onClick={logout} className="text-[10px] font-black uppercase tracking-widest text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 px-5 py-2.5 rounded-xl transition-all duration-300 hover:scale-105 active:scale-95">Exit</button>
            </div>
          ) : (
            <div className="flex items-center gap-4">
              <Link to={ROUTES.LOGIN} className="text-[10px] font-black uppercase tracking-widest px-6 py-2.5 rounded-xl transition-all duration-300 no-underline hover:scale-105 active:scale-95 theme-text-sub hover:theme-text hover:theme-bg">Sign In</Link>
              <Link to={ROUTES.SIGN_UP} className="btn-primary text-[10px] font-black uppercase tracking-[0.15em] px-8 py-3.5 rounded-xl no-underline shadow-xl shadow-teal-500/20 hover:scale-105 active:scale-95 transition-all duration-300 hover:shadow-teal-500/40">Join Free</Link>
            </div>
          )}
        </div>
      ...
      {menuOpen && (
        <div className="md:hidden theme-surface border-t theme-border px-8 py-8 flex flex-col gap-2 shadow-2xl animate-fade-in">
          {NAV_LINKS.map(link => (
            <a key={link.label} href={link.href} onClick={() => setMenuOpen(false)} className="block py-4 text-xs font-black uppercase tracking-widest theme-text no-underline border-b theme-border last:border-0 hover:text-teal-600 transition-colors">{link.label}</a>
          ))}
          <div className="pt-6 mt-4 flex flex-col gap-6">
            ...
            {isAuthenticated ? (
              ...
              <div className="flex flex-col gap-4">
                <Link to={ROUTES.LOGIN} onClick={() => setMenuOpen(v => !v)} className="text-xs font-black uppercase tracking-widest theme-text no-underline hover:text-teal-600 transition-colors">Sign In</Link>
